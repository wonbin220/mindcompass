...
9. 감정 기록 화면에서는 사용자 입력 감정(`primaryEmotion`, `emotionIntensity`)과 AI 분석 감정(`aiPrimaryEmotion`, `aiEmotionIntensity`, `aiSummary`, `aiConfidence`)을 구분해서 표시할 수 있다.
...
## 13-6. 화면 계약 메모
- Diary 상세 화면은 아래 필드를 함께 사용하면 가장 자연스럽다.
  - 사용자 입력 감정: `primaryEmotion`, `emotionIntensity`
  - AI 분석 감정: `aiPrimaryEmotion`, `aiEmotionIntensity`, `aiSummary`, `aiConfidence`
  - Safety 해석: `riskLevel`, `riskScore`, `riskSignals`, `recommendedAction`
- 즉 diary 상세 응답은 단순 저장 결과가 아니라, 화면이 바로 표시할 수 있는 AI 분석 카드 데이터까지 포함하는 계약으로 보는 것이 맞다.
