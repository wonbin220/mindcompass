...
이 문서는 모바일 앱이 `backend-api`만 호출하도록 화면별 API 연결 순서를 정리한 문서다.

전제:
- 모바일 앱은 `backend-api`만 호출한다.
- `ai-api`는 Spring Boot 내부 호출 전용이다.
...
실제 화면 버튼, API client, 상태 관리, DTO 파싱 코드를 수정하려면 모바일 앱 코드가 필요하다.

즉:
- 앱 코드가 없으면 UI 코드는 못 바꾼다.
- 앱 코드가 없어도 API 계약/연동 순서/응답 해석 문서는 준비할 수 있다.
- 앱 코드가 제공되면 그때부터 실제 화면 연동 작업을 바로 진행할 수 있다.
