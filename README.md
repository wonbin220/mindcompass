...
1. 왜 `Spring Boot + Spring AI 내부 서버` 구조로 가는지 이해한다.
2. `backend-api`, `ai-api`, `ai-api-fastapi`가 각각 무엇을 맡는지 구분한다.
3. Spring AI 비교 실험을 할 때 어떤 기준으로 구조를 유지해야 하는지 정리한다.
...
- `ai-api-fastapi`는 기존 FastAPI 비교 서버로 남겨뒀다.
- `ai-api`는 새 Spring AI 기반 내부 AI 서버다.
...
- 앱은 `backend-api`만 호출한다.
- `backend-api`는 공개 API, 인증, 저장, 비즈니스 흐름의 중심이다.
- `ai-api`는 Spring AI 기반 내부 AI 서버다.
- `ai-api-fastapi`는 기존 FastAPI 구현을 비교용으로 유지하는 서버다.
- `backend-api`는 필요에 따라 `ai-api` 또는 `ai-api-fastapi`를 내부적으로 호출할 수 있다.

즉:

앱 -> `backend-api` -> `ai-api` 또는 `ai-api-fastapi`
...
- Spring AI 기반 `ai-api`는 **AI 분석과 추론 담당**
- 필요하면 `ai-api-fastapi`를 **비교 실험용 내부 서버**로 함께 둔다
- Spring Boot가 내부적으로 AI 서버를 호출한다
...
즉, `ai-api`는 **Spring 생태계 안에서 AI 실험을 담당하는 내부 추론 서버**다.

## 3-3. ai-api-fastapi는 왜 남겨두는가

`ai-api-fastapi`를 유지하는 이유는 단순하다.

- 기존 FastAPI 구현을 버리지 않고 비교할 수 있다.
- 같은 요청/응답 계약으로 품질을 비교할 수 있다.
- 응답 속도, 운영 편의성, 코드 생산성을 비교할 수 있다.
- Spring AI 전환이 실제로 이득인지 확인할 수 있다.

즉, `ai-api-fastapi`는 운영 주력이 아니라
**비교 기준점(reference implementation)** 역할이다.

## 3-4. 왜 앱이 AI 서버를 직접 부르면 안 되는가

앱이 `ai-api`나 `ai-api-fastapi`를 직접 호출하면 다음 문제가 생긴다.
...
그래서 앱은 **무조건 `backend-api`만 호출**하게 만든다.

이렇게 해야 앱 입장에서는 진입점이 하나라서 단순하고,
...
- Spring 생태계에서 프롬프트 / 모델 설정 실험
- 향후 Spring AI 기반 도구 호출, 메모리, RAG 확장
...
- `ai-api`는 Spring AI 기반 내부 추론 서버다.
- `ai-api-fastapi`는 비교용 기준 서버다.
- 모바일 앱은 AI 서버를 직접 호출하면 안 된다.
